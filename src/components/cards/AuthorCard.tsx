...
export default function AuthorCard({author}: Props) {
  ...
  return (
    <div className="w-80 h-36 relative group overflow-clip cursor-pointer" onClick={handleClick}>
        ...
        <div className="h-20 w-full relative top-1/2 -translate-y-4 bg-gray-500 rounded-md flex justify-center items-center ">
          <p className='text-white font-bold text-2xl relative group-hover:translate-y-2 duration-300 transition-all'>{fullName}</p>
        </div>
    </div>
  )
}
